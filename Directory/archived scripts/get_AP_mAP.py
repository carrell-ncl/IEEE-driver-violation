...
def calculate_AP(class_ID=0, IOU_threshold=0.4):
    ...
    # For recall we divide true positive by length of DF (total GT)  
...
AP_plate = calculate_AP(1, IOU_threshold=0.7)

AP_plate[0]

thresh = [0.4, 0.5, 0.6, 0.7]

for threshold in thresh:
    print(f'Thresh {threshold}= ', calculate_AP(1, IOU_threshold=threshold))
...
average_precision_score(rec, prec, average='weighted')

sum((rec[:-1]-rec[1:])* prec[:-1])
rec[:-1]
rec[1:]
...
# =============================================================================
# Display test images with GT and predicted boxes
# =============================================================================
...
# img_path = 'C:\\Users\\Steve\\Desktop\\deeplearning\\directory\\OIDv4_ToolKit-master\\OID\\Dataset/test\\Phone/mar6.JPG'
# cor1 = tuple(gt_data[-3][0][:2])
# cor2 = tuple(gt_data[-3][0][2:4])
# cor1
...
def check_images(class_id):
    count = 0
    if class_id == 0:
        directory = 'C:\\Users\\Steve\\Desktop\\deeplearning\\directory\\OIDv4_ToolKit-master\\OID\\Dataset/test\\Phone'
    else:
        directory = 'C:\\Users\\Steve\\Desktop\\deeplearning\\directory\\OIDv4_ToolKit-master\\OID\\Dataset/test\\Vehicle_registration_plate'
        
    gt_class = [gt for gt in gt_data if gt[0][-1]==class_id]
    for file, coors in zip(os.listdir(directory), gt_data):
        if os.path.basename(directory) == 'Phone':
            if file.endswith(".JPG") or file.endswith(".JPEG") or file.endswith(".jpg"):
                image_path = os.path.join(directory, file)
                print(image_path)
                cor1 = tuple(gt_class[count][0][:2])
                cor2 = tuple(gt_class[count][0][2:4])
                ...
                count+=1
        if os.path.basename(directory) == 'Vehicle_registration_plate':
